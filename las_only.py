...
def las_train(model, train_loader, optimizer, criterion, device):
    model.train()
    ...
    for i, data in enumerate(train_loader):
        ...
        logits = model(inputs, inputs_lengths, targets, 0.5, False)
...
def main():
    ...
    with open("./las_train.txt", "w") as f:
        f.write(yaml_name)
        f.write('\n')
        f.write('\n')
        f.write("학습 시작")
    ...
    for epoch in range(config.training.begin_epoch, config.training.end_epoch):
        ...
        with open("./las_train.txt", "a") as ff:
